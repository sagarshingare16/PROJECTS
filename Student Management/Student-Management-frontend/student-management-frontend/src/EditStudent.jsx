...
const EditStudent = () => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        const result = await axios.get(`http://localhost:8181/api/v1/student/get-student-details/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:8181/api/v1/student/update-student${id}`, student);
      navigate('/'); 
    } catch (error) {
