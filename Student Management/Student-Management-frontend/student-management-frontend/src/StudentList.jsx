...
const StudentList = () => {
  ...
  const fetchStudents = async () => {
    try {
      const response = await axios.get('http://localhost:8181/api/v1/student/get-all-students');
  ...
  const deleteStudent = async (id) => {
    try {
      await axios.delete(`http://localhost:8181/api/v1/student/delete-student/${id}`);
      fetchStudents(); 
    } catch (error) {
